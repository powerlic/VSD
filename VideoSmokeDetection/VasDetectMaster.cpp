...
void VasDetectMaster::DetectFrameMerged(shared_ptr<Detecter> ptr_detecter, const Mat &frame, const int64_t &pts, const int64_t &No)
{
	...
	{
		//LOG(INFO) << "reg size " << utified_reg_rects.size() << endl;
		reg_signal_(ptr_detecter->service_id_.c_str(), ptr_detecter->show_frame_, utified_reg_rects, reg_size.width, reg_size.height, utified_reg_scores, utifiled_reg_labels);
...
bool VasDetectMaster::SetShowRegFrame(const char* service_id, bool is_show)
{
	shared_ptr<Detecter> ptr_detecter = GetDetecterPtr(service_id);
	if (ptr_detecter)
	{
