...
VsdMaster::VsdMaster(const char*caffe_model_path_proto_file)
{
	...
	ptr_vas->SetRegResultProcessFunction(boost::bind(&VsdMaster::RegResultProcessFun, this, _1, _2, _3, _4, _5, _6, _7));
...
*/
void VsdMaster::RegResultProcessFun(const char* service_id, Mat &frame, const vector<CvRect> &reg_rects,
...
VsdNvrMaster::VsdNvrMaster(const char*caffe_model_path_proto_file, const char*nvr_access_proto_file)
{
	...
	ptr_vas_nvr_->SetRegResultProcessFunction(boost::bind(&VsdNvrMaster::RegResultProcessFun, this, _1, _2, _3, _4, _5, _6,_7));
...
void VsdNvrMaster::RegResultProcessFun(const char* service_id,Mat &frame, const vector<CvRect> &reg_rects,
	const uint32_t dst_width, const uint32_t dst_height,
	const vector<float> &scores, const vector<string> &labels)
{
	LOG(INFO) << "---------service_id " << service_id << " reg_results------" << endl;
...
VsdSsdMaster::VsdSsdMaster(const char*ssd_model_path_proto_file)
{
	ptr_vas_ssd_ = shared_ptr<VasSsdP>(CreateVasSsdP(ssd_model_path_proto_file));
	ptr_vas_ssd_->SetRegResultProcessFunction(boost::bind(&VsdSsdMaster::RegResultProcessFun, this, _1, _2, _3, _4, _5, _6, _7));
	//ptr_vas_nvr_->SetServiceCommandReplyProcessFunction(boost::bind(&VsdNvrMaster::ServiceCommandReplyProcessFun, this, _1, _2, _3));
	ptr_vas_ssd_->SetStreamStatusErrorFunction(boost::bind(&VsdSsdMaster::StreamErrProcessFun, this, _1, _2));
}
VsdSsdMaster::~VsdSsdMaster()
{
	ptr_vas_ssd_.reset();
}

void VsdSsdMaster::RegResultProcessFun(const char* service_id, Mat &frame, const vector<CvRect> &reg_rects,
	const uint32_t dst_width, const uint32_t dst_height,
	const vector<float> &scores, const vector<string> &labels)
{
	LOG(INFO) << "---------service_id " << service_id << " reg_results------" << endl;
...
void VsdSsdMaster::StreamErrProcessFun(const char *service_id, vas::StreamStatus status)
{
	switch (status)
	{
	case vas::STREAM_UNKNOWN:
